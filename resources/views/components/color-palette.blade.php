<div x-data="{ hover: false }">
    <p
        class="md:space-x-1 space-y-1 md:space-y-0 fixed -bottom-5 right-0 flex items-center justify-center z-50 mb-10 mr-10 inline-block">
        <svg :class="hover ? 'w-12 h-12 opacity-100': 'w-10 h-10 opacity-60'" @click="hover = !hover"
            class="hover:opacity-100 hover:w-12 hover:h-12 inline-block leading-tight cursor-pointer" version="1.1"
            id="Layer_1" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false"
            aria-controls="collapseExample" xmlns="http://www.w3.org/2000/svg"
    ...
    <div x-show="hover" :class="{ 'block':hover, 'hidden': !hover }" class="hidden">
        <x-admin.dark-mode />
        <x-admin.main-color />
    </div>
</div>
