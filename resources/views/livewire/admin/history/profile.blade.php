<div>
    ...
    <div class="px-5 pt-5 mt-5 intro-y box">
        <div class="flex flex-col pb-5 -mx-5 border-b lg:flex-row border-slate-200/60 dark:border-darkmode-400">
            ...
            <div
                class="flex-1 px-5 pt-5 mt-6 border-t border-l border-r lg:mt-0 border-slate-200/60 dark:border-darkmode-400 lg:border-t-0 lg:pt-0">
                <div class="flex flex-col items-center justify-center prose lg:items-start">
                    <div class="flex items-center truncate sm:whitespace-normal">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <span class="m-1 font-bold"> Tipo de sangre: </span>
                        {{ $person->patient->blood_type ? $person->patient->blood_type : 'No registrado' }}
                    </div>
                    <div class="flex items-center mt-3 truncate sm:whitespace-normal">
                        ...
                        <span class="m-1 font-bold"> Peso: </span>
                        {{ $person->patient->weight ? $person->patient->weight : 'No registrado' }}
                    </div>
                    <div class="flex items-center mt-3 truncate sm:whitespace-normal"> <svg
                            xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg> <span class="m-1 font-bold"> Altura: </span>
                        {{ $person->patient->height ? $person->patient->height : 'No registrado' }} </div>
                    <div class="flex items-center mt-3 truncate sm:whitespace-normal"> <svg
                            xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg> <span class="m-1 font-bold"> IMC:
                        </span>
                        {{ round(floatval($person->patient->weight) / pow(2, floatval($person->patient->height)), 2) }}
                    </div>
                </div>
            </div>
        </div>
