...
<div x-data="{ modal: false }">
            ...
            <div class="mt-5">
                ...
                @else
                    @foreach ($departments as $department)
                        @if ($department->name !== 'Administración' && $department->name !== 'Enfermería')
                            <button wire:click="tickets('{{ $department->id }}')"
                                class="w-40 mb-2 mr-1 btn btn-primary">
                                {{ $department->name }}
                            </button>
                        @endif
                    @endforeach
                        ...
                        <tbody>
                            @foreach ($tickets as $ticket)
                                <tr>
                                    ...
                                    @if ($ticket->status == '0')
                                        <td>
                                            <span @click="modal = true, $wire.set('aux', {{ $ticket->id }}), $wire.set('ticket', {{ $ticket->id }}),
                                                $wire.getPatients()"
                ...
                <div x-transition:enter="transition ease-out duration-150"
                    ...
                    <footer
                        ...
                        <div>
                            ...
                            <button wire:click="updateTicket"
    ...
    @else
        <div class="p-10">
            <div class="mb-2 alert alert-secondary show" role="alert">
                <div class="flex items-center">
                    <div class="text-lg font-medium">No existen Especialidades registradas.</div>
                </div>
                <div class="mt-3">
                    <p>
                        Para registrar una esoecialidad diríjase a <a class="font-semibold text-blue-600 underline"
                            href="{{ route('admin.dashboard') }}"> Tabla de Departamentos. </a> </p>
                </div>
            </div>
        </div>
    @endif
